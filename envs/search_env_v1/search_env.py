...
class SearchEnv(gym.Env):
    ...
    def __init__(self, map_size, vision_distance):
        ...
        self.observation_space = spaces.Box(low=0, high=1, shape=(self._map_size, self._map_size, 3))
    ...
    def step(self, action):
        ...
        return np.dstack((self._exploration_map, self._object_map, self._robot_map)), reward, done, info
    ...
    def reset(self):
        ...
        self._master_object_map = np.array([[0] * self._map_size for _ in range(self._map_size)]) # The env's reference for the world
        self._exploration_map   = np.array([[0] * self._map_size for _ in range(self._map_size)]) # if a block has been explored
        self._object_map        = np.array([[0] * self._map_size for _ in range(self._map_size)]) # if an object exists in the explored space
        self._robot_map         = np.array([[0] * self._map_size for _ in range(self._map_size)]) # the last known positions of the robots
        ...
        return np.dstack((self._exploration_map, self._object_map, self._robot_map))
